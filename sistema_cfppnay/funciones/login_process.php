...
if (isset($_SESSION['error'])) {
    // Es buena idea no imprimir HTML directamente antes de redireccionar, pero lo mantenemos para tu lógica
...
$host = "127.0.0.1";
$port = "3307"; // <-- Aquí agregamos el puerto
$db = "cefppenay";
$user = "root";
$pass = "";

try {
    // <-- Agregamos port=$port a la cadena de conexión
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$email || !$password) {
        ...
        header("Location: ../login.php"); // <-- Salimos de funciones/ y vamos a login.php
        exit;
    }
    ...
    if ($usuario) {
        // Verifica contraseña usando password_verify
        if (password_verify($password, $usuario['password_hash'])) {
            ...
            header("Location: ../interfaces/panel_control.php"); // <-- Salimos de funciones/ y entramos a interfaces/
            exit;
        } else {

...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (!empty($idGlobalCliente) && !empty($cantidad_abonoP) && !empty($metodo_pagoP) && !empty($producto)) {
        ...
        try {
            ...
            // Si el producto es "Enganche 100+100", actualizar la tabla folios
            //final
            if ($producto === "Enganche 100+100") {
                ...
                $sql_promociones = "SELECT promociones FROM folios WHERE id_folio = ?";
                $stmt = mysqli_prepare($conn, $sql_promociones);
                if (!$stmt) {
                    throw new Exception("Error al preparar la consulta para obtener promociones: " . mysqli_error($conn));
                }
                mysqli_stmt_bind_param($stmt, "i", $aboprodfolio);
                mysqli_stmt_execute($stmt);
                mysqli_stmt_bind_result($stmt, $promociones);
                mysqli_stmt_fetch($stmt);
                mysqli_stmt_close($stmt);

                // Si las promociones existen y "Enganche 100+100" no está ya en la lista, agregarlo
                if ($promociones) {
                    $promociones_array = explode(",", $promociones);
                    if (!in_array("Enganche 100+100", $promociones_array)) {
                        // Agregar "Enganche 100+100" al final si no está presente
                        $promociones .= ",Enganche 100+100";
                    }
                } else {
                    // Si no hay promociones, simplemente agregar "Enganche 100+100"
                    $promociones = "Enganche 100+100";
                }

                // Actualizar el campo "promociones" en la tabla folios
                $sql_update_promociones = "UPDATE folios SET promociones = ? WHERE id_folio = ?";
                $stmt = mysqli_prepare($conn, $sql_update_promociones);
                if (!$stmt) {
                    throw new Exception("Error al preparar la consulta para actualizar promociones: " . mysqli_error($conn));
                }
                mysqli_stmt_bind_param($stmt, "si", $promociones, $aboprodfolio);
                if (!mysqli_stmt_execute($stmt)) {
                    throw new Exception("Error al ejecutar la consulta de actualización de promociones: " . mysqli_stmt_error($stmt));
                }
                mysqli_stmt_close($stmt);
            }

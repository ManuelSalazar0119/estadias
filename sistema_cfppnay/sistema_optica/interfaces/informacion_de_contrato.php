...
<div id="modalTicket" class="modalT">
    <div class="modal-content-t">
        ...
        <p><strong>Cantidad Abonada:</strong> <span id="modalCantidadAbono"></span></p>
        <p><strong>Método de Pago:</strong> <span id="modalMetodoPago"></span></p>
        <p><strong>Saldo Anterior:</strong> <span id="modalSaldoAnterior"></span></p>
        <p style="color:red;"><strong>Nuevo Saldo:</strong> <span id="modalNuevoSaldo"></span></p>
        <p><strong>Cobrador:</strong> <span id="modalNombreCobrador"></span></p>

        <!-- Botón para reintentar impresión -->
        <button id="btnReintentarImpresion" onclick="reintentarImpresion()">Impresión</button>
        <button id="cerrarModalT">Cerrar</button>
    </div>
</div>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    var btnReintentarImpresion = document.getElementById('btnReintentarImpresion');
    ...
    var cerrarModalT = document.getElementById("cerrarModalT");
    ...
    cerrarModalT.onclick = function(){
        modalTicket.style.display = "none";
        localStorage.removeItem("datosAbonoPendiente"); // Eliminar los datos al imprimir correctamente
        location.reload();
    }
    ...
    btnImprimirAbono.onclick = function() {
        ...
        var saldoAnterior = saldoNuevo; // Guardamos el saldo actual antes de cualquier operación

    if (liquidar) {
        const descuento = 300;

        // Asegurarnos que el descuento no exceda el saldo
        if (descuento > saldoNuevo) {
            throw new Error("El descuento no puede ser mayor al saldo actual");
        }

        cantidadAbono = saldoNuevo - descuento;
        var nuevoSaldo = 0; // Ya está liquidado, no debe quedar saldo
    } else {
        // Pago normal en abonos
                        ...
                        var urlGenerarTicket = isMobile 
                            ...
                            if (!isMobile) {
                                ...
                                function guardarDatosAbono() {
                                    ...
                                    localStorage.setItem("datosAbonoPendiente", JSON.stringify(datosAbono));
                                }

                                
                                // Función para intentar la impresión nuevamente
                                function reintentarImpresion() {
                                    const datosAbono = JSON.parse(localStorage.getItem("datosAbonoPendiente"));
                                    if (!datosAbono) {
                                        alert("No hay datos pendientes de impresión.");
                                        return;
                                    }
                                    let urlParams3 = new URLSearchParams(datosAbono);
                                    let urlParams = new URLSearchParams({
                                    idGlobalCliente: datosAbono.idGlobalCliente,
                                    folio_encode: datosAbono.folioEncode,
                                    nombre_cliente: datosAbono.nombreCliente,
                                    cantidad_abono: datosAbono.cantidadAbono,
                                    metodo_pago: datosAbono.metodoPago,
                                    nuevo_saldo: datosAbono.nuevoSaldo,
                                    aboprodfolio: datosAbono.aboprodfolio,
                                    nombre_cobrador: datosAbono.nombreCobrador,
                                    saldo_anterior: datosAbono.saldoAnterior
                                });
                                    let urlGenerarTicket = '../funciones/generar_ticket_movil.php';
                                    
                                    fetch(urlGenerarTicket + '?' + urlParams.toString())
                                    .then(response => response.json())
                                    .then(data => {
                                        //console.log("Respuesta del servidor:", data); // Muestra la respuesta real en la consola
                                        if (data.success) {
                                            ...
                                            alert("Error al generar el ticket. Inténtalo nuevamente.");
                                        }
                                    })
                                    .catch(error => {
                                        console.error("Error:", error);
                                        alert("No se pudo conectar con la impresora. Intenta nuevamente.");
                                    });
                                }
                                    btnReintentarImpresion.onclick = reintentarImpresion;
                                function abrirModalTicket() {
                                const datosAbono = JSON.parse(localStorage.getItem("datosAbonoPendiente"));
                                if (!datosAbono) return;

                                // Llenar los datos en el modal
                                document.getElementById("modalNombreCliente").textContent = datosAbono.nombreCliente;
                                document.getElementById("modalFolio").textContent = datosAbono.folioEncode;
                                document.getElementById("modalCantidadAbono").textContent = datosAbono.cantidadAbono;
                                document.getElementById("modalMetodoPago").textContent = datosAbono.metodoPago;
                                document.getElementById("modalNuevoSaldo").textContent = datosAbono.nuevoSaldo;
                                document.getElementById("modalSaldoAnterior").textContent = datosAbono.saldoAnterior;
                                document.getElementById("modalNombreCobrador").textContent = datosAbono.nombreCobrador;
                                ...
                                abrirModalTicket(); // Abre el modal al fallar la impresión
...
<!-- Modal de Abono -->
        <div id="modalAbono" class="modal-abono" style="display: none;">
            <div class="modal-content-abono">
                ...
                <br>
                <div class="form-row">
                    <p><b><label for="liquidar">¿Liquidar?
